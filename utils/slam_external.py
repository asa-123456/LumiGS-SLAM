...
def remove_points(to_remove, params, variables, optimizer):
    to_keep = ~to_remove
    # Exclude camera parameters and GRU parameters (per-frame, not per-gaussian)
    keys = [k for k in params.keys() if k not in ['cam_unnorm_rots', 'cam_trans', 'gru_affine', 'gru_tone']]
    for k in keys:
        group = [g for g in optimizer.param_groups if g['name'] == k][0]
        stored_state = optimizer.state.get(group['params'][0], None)
        if stored_state is not None:
...
def densify(params, variables, optimizer, iter, densify_dict):
    if iter <= densify_dict['stop_after']:
        ...
        if (iter >= densify_dict['start_after']) and (iter % densify_dict['densify_every'] == 0):
            ...
            # Exclude camera parameters and GRU parameters (per-frame, not per-gaussian)
            new_params = {k: v[to_clone] for k, v in params.items() if k not in ['cam_unnorm_rots', 'cam_trans', 'gru_affine', 'gru_tone']}
            ...
            # Exclude camera parameters and GRU parameters (per-frame, not per-gaussian)
            new_params = {k: v[to_split].repeat(n, 1) for k, v in params.items() if k not in ['cam_unnorm_rots', 'cam_trans', 'gru_affine', 'gru_tone']}
