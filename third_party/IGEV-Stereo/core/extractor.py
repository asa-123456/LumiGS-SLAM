...
class Feature(SubModule):
    def __init__(self):
        ...
        # NOTE: avoid any online download (HuggingFace Hub) by disabling pretrained
        pretrained = False
        model = timm.create_model('mobilenetv2_100', pretrained=pretrained, features_only=True)

        # Load pretrained weights from a local checkpoint instead of downloading
        ckpt_path = "/home/zsh107552403865/.cache/huggingface/hub/models--timm--mobilenetv2_100/snapshots/000000/mobilenetv2_100_ra-b33bc2c4.pth"
        state_dict = torch.load(ckpt_path, map_location="cpu")
        # The checkpoint is an OrderedDict mapping parameter names to tensors.
        # Some entries (e.g. classifier) are not used by the feature-only model, so load non-strictly.
        model.load_state_dict(state_dict, strict=False)
        layers = [1,2,3,5,6]
        chans = [16, 24, 32, 96, 160]
        self.conv_stem = model.conv_stem
        self.bn1 = model.bn1
        # Some timm versions do not expose `act1` on EfficientNetFeatures,
        # so we explicitly define the activation to avoid attribute errors.
